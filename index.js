...
let countFiles = 0;
let timeExtractZip = 0;
let timeParseXML2JSON = 0;
let timeMapJSON = 0;
let totalLines = 0;
...
async function extractZip() {
	...
	if (responseMonth) {
		...
		const ficheroZip = ficheroZIP
			...
			.replace(
				'PROCCESS',
				process == 1 ? 'licitacionesPerfilesContratanteCompleto3' : 'contratosMenoresPerfilesContratantes'
			)
		...
		countFiles = await zip.extract(null, './extracted/atom');
		const stop = Date.now();
		timeExtractZip = ((stop - start) / 60000).toFixed(2);
		await zip.close();
		return Promise.resolve(true);
	} else {
		responseMonth = readline.question('Ingresa el mes 09-10-11...\n');
		extractZip();
	}
}
...
async function parseXML2JSON() {
	...
	for (const fichero of archivos) {
		...
		if (fs.existsSync(pathFile) && fs.lstatSync(pathFile).isFile()) {
			...
			console.log(`${contador}/${countFiles} ${fichero} / lineas ${new Intl.NumberFormat('es-Es').format(countLines)}`);
	...
	const stop = Date.now();
	timeParseXML2JSON = ((stop - start) / 60000).toFixed(2);
}
...
function mapJSON() {
	...
	archivos.forEach((fichero) => {
		...
		console.log(`${contador}/${countFiles} ${fichero}`);
		contador++;
	...
	pathResults =
		pathResultsParam.replace('FOLDER', process == 1 ? 'licitaciones' : 'contratos menores') + `/${responseMonth}`;
	// Guarda todos los resultados
	saveFinalJson(arrayFinal);
	const stop = Date.now();
	timeMapJSON = ((stop - start) / 60000).toFixed(2);
}
...
function saveFinalJson(arrayFinal) {
	fs.rmdirSync('./extracted', { recursive: true, force: true }, (error) => {
		console.error('Error: ', error);
	});
	...
	console.log(`Tiempo para ejecutar extractZip()    = ${timeExtractZip} minutos`);
	console.log(`Tiempo para ejecutar parseXML2JSON() = ${timeParseXML2JSON} minutos`);
	console.log(`Tiempo para ejecutar mapJSON()       = ${timeMapJSON} minutos`);
	console.log(`Total lineas XML analizadas          = ${new Intl.NumberFormat('es-Es').format(totalLines)}`);
...
function searchRepeat(arrayFinal) {
	...
	return {
		listRepeat: objectRepeat.repeat.length,
		listRepeatMajor: objectRepeat.repeatMajor.length,
		listNoRepeat: objectRepeat.noRepeat.length
	};
}
...
function logFinal(totalLicitaciones, sinRepeticion, repetidos, mayores) {
	...
	const logFinal = {
		'Tiempo para ejecutar extractZip()': `${timeExtractZip} minutes`,
		'Tiempo para ejecutar parseXML2JSON()': `${timeParseXML2JSON} minutes`,
		'Tiempo para ejecutar mapJSON()': `${timeMapJSON} minutes`,
		'Total lineas XML analizadas': `${new Intl.NumberFormat('es-Es').format(totalLines)}`,
...
async function readLines(name) {
	...
	return new Promise((resolve) => {
		...
		let count = 0;
		readInterface.on('line', function () {
			count++;
		});
